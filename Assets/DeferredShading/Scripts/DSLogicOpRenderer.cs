...
public class DSLogicOpRenderer : MonoBehaviour
{
	...
	{
		if (rtRDepth == null) {
			rtRDepth = DSRenderer.CreateRenderTexture((int)dscam.cam.pixelWidth, (int)dscam.cam.pixelHeight, 32, RenderTextureFormat.RHalf);
		}
		if (DSAnd.instances.Count > 0 && rtAndRDepth==null)
		{
			Camera cam = dscam.cam;
			rtAndRDepth = DSRenderer.CreateRenderTexture((int)dscam.cam.pixelWidth, (int)dscam.cam.pixelHeight, 32, RenderTextureFormat.RHalf);
