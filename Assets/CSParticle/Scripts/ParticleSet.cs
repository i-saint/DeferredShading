...
public class MPParticleSetImplGPU : IMPParticleSetImpl
{
    ...
    ComputeBuffer m_cb_trail_vertices;
    ...
    public override void OnDisable()
    {
        if (m_cb_trail_vertices != null)
        {
            m_cb_trail_vertices.Release();
            m_cb_trail_history.Release();
            m_cb_trail_entities.Release();
            m_cb_trail_params.Release();
        }
    ...
    public override void Start()
    {
        ...
        if (pset.m_enable_trail)
        {
            DispatchTrailKernel(0, pset.maxParticles % BLOCK_SIZE);
        }
    }
    ...
    void DispatchTrailKernel(int i, int n)
    {
        ...
        ComputeShader cs = world.csTrail;
        cs.SetBuffer(i, "particles", cbParticles[0]);
        cs.SetBuffer(i, "params", m_cb_trail_params);
        cs.SetBuffer(i, "entities", m_cb_trail_entities);
        cs.SetBuffer(i, "history", m_cb_trail_history);
        cs.SetBuffer(i, "vertices", m_cb_trail_vertices);
        cs.Dispatch(i, n, 1, 1);
    }
    ...
    public override void DepthPrePass()
    {
        ...
        if (matGBuffer==null || !pset.m_enable_gbuffer || pset.processGBufferCollision || !pset.depthPrePass) { return; }
    ...
    {
        Material matGBuffer = pset.matParticleGBuffer;
        if (matGBuffer != null || !pset.m_enable_gbuffer)
        {
            matGBuffer.SetBuffer("vertices", wimpl.cbCubeVertices);
            matGBuffer.SetBuffer("particles", cbParticles[0]);
